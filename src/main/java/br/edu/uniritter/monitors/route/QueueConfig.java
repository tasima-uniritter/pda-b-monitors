...
public class QueueConfig {
    ...
    AMQPConnectionDetails securedAmqpConnection() {
        ...
        String amqpPwd = dotenv.get("AMQP_SERVICE_PASSWORD");
        if (amqpPwd.length() == 0) {
            throw new IllegalArgumentException("Missing AMQP_SERVICE_PASSWORD environment variable");
        }

        String uri = AMQP_SERVICE_PROTOCOL + "://" + AMQP_SERVICE_HOST + ":" + AMQP_SERVICE_PORT;
        return new AMQPConnectionDetails(uri, AMQP_SERVICE_USERNAME, amqpPwd);
    }
}